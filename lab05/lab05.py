...
def closer_city(lat, lon, city1, city2):
    ...
    if distance(dest, city1) < distance(dest, city2):
...
def sprout_leaves(t, values):
    ...
    new_branches = [sprout_leaves(b, values) for b in old_branches]
...
def add_trees(t1, t2):
    ...
    if len(branches_t1) < len(branches_t2):
        t1, t2 = t2, t1
        branches_t1, branches_t2 = branches_t2, branches_t1
    while len(branches_t1) != len(branches_t2):
        branches_t2.append(tree(0))
    return tree(node, [add_trees(branches_t1[i], branches_t2[i]) for i in range(len(branches_t1))])
